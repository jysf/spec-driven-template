...
7. **Disposition the stage-owned signals.** Open `/guidance/signals.yaml`
   ...
   Add any new lesson the reflections above surfaced (don't codify it now if
   it's below bar — record it as `watch`). See `docs/signals.md`.

8. Propose updates to /AGENTS.md, /guidance/*, or templates — including
   landing any lesson you marked `codified` in step 7.
...
- If template/constraint/decision updates mentioned but NOT made now,
  record them in /guidance/signals.yaml (type: lesson with its N-count for
  a recurring coding pattern; type: process-debt for tooling friction) so a
  close forces the decision. If made now, propose the edits directly.
