...
> *disagree*: the template's **derived** roadmap (`just roadmap` reads framed
> `STAGE-*.md` + planned `## Stage Plan` checkboxes, shipped v0.6.14) and the
> **declared** roadmap the `standup` tracker invented and reads itself (a
> `roadmap:` brief front-matter block of pillars + `resume_when`; see the
> [standup deep-dive](../../)). Codifying = giving both ends *one* structure.
> **Do not build cold** — validate the schema on the 3-disciplined-project
> harvest and by actually wiring standup to consume the new `--json`. The
> smallest real increment (Phase 1 below) ships value to standup immediately.
...
1. **`goal` vs DEC-009 `value_metric` — one concept or two?** Is a roadmap `goal`
   item the *same* object a `value_metric` attaches to, or a lighter pointer to
   it? Resolve alongside the Goals/Plans layer; they likely converge.
2. **Merge/dedupe rules for declared-references-a-stage.** When `item: STAGE-004`
   also exists as a `## Stage Plan` row or a file, which record wins, and does the
   declared `horizon` override the derived ordering?
3. **Horizon vocabulary final?** Is `now|next|later` enough, or is a fourth bucket
   (`someday`/`icebox`) worth it? Keep it minimal until a project needs more.
4. **Does the template's own `docs/ROADMAP.md` adopt this structure** dogfood-style,
   or stay hand-written prose? (It's the template's *meta* roadmap; the codified
   structure is for projects built *with* the template — likely keep it prose.)
5. **Validate warn-only or hard?** Follow the `activity` precedent — an
   unrecognized `kind`/`horizon` should be advisory (open-set discipline), not a
   gate failure.

## Rollout sketch (once accepted)

- **Phase 1 (template, non-breaking):** bless the `roadmap:` block in
  `schema-reference.md` + the brief scaffold; parse it in `_lib.sh`; `just
  roadmap` merges declared + derived and emits both in `--json`; `just validate`
  warns-only on unrecognized `kind`/`horizon`. Add `scripts/test.sh` coverage.
